...
CText CSharedAllocator::heap_head_t::MWatchDogMutexName() const
{
	if (MIsWatchDogExist()) 
	{
		...
		sprintf(_mutex_name, "Wd_%d_%d", FWacthDogPid, FWacthDogTid);
		return _mutex_name;
	}
	return CText();
}
...
void CSharedAllocator::MCreateHeap(void* aBase, block_size_t aSize,size_t aReserv)
{
	...
	//the first is - pid, the second random string
	{
		...
		char _mutex_name[1024];
		sprintf(_mutex_name,"sm_%d_al_%s", get_pid_optimized(), _rand.c_str());

		size_t _name_len = strlen(_mutex_name);
		_name_len =
				_name_len <= (sizeof(_p_head->FSharedMutex) - 1) ?
						_name_len : (sizeof(_p_head->FSharedMutex) - 1);
		memcpy(_p_head->FSharedMutex, _mutex_name, _name_len);
	...
	}
	{
		...
		_rand=get_random_str(10);
		char _mutex_name[1024];
		sprintf(_mutex_name,"sm_%d_fr_%s", get_pid_optimized(), _rand.c_str());

		size_t _name_len = strlen(_mutex_name);
		_name_len =
				_name_len <= (sizeof(_p_head->FFreeSem) - 1) ?
						_name_len : (sizeof(_p_head->FFreeSem) - 1);
		memcpy(_p_head->FFreeSem, _mutex_name, _name_len);
